...
def screensaver():
    ...
    sub = pygame.display.get_surface().subsurface(rect)
   
    buffer = io.BytesIO()
    pygame.image.save(sub, buffer)
    buffer.seek(0)
    ...
    blur_image.save(blur_buffer, format='PNG')
    blur_buffer.seek(0)
    blurred_surface = pygame.image.load(blur_buffer)

    return blurred_surface
    
def pause():
    display.blit(screensaver(), (0, 0))
